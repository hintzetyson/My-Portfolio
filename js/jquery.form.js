 ...
 */
  $.fn.ajaxSubmit = function (options) {
    ...
    let method; let action; let url; const
      $form = this;
    ...
    const fileInputs = $('input[type=file]:enabled[value!=""]', this);
    ...
    // clear element array
    for (let k = 0; k < elements.length; k++) elements[k] = null;
    ...
    function fileUploadIframe(a) {
      const form = $form[0]; let el; let i; let s; let g; let id; let $io; let io; let xhr; let sub; let n; let timedOut; let
        timeoutHandle;
      ...
      function doSubmit() {
        ...
        const t = $form.attr2('target'); const
          a = $form.attr2('action');
      ...
      let data; let doc; let domCheckCount = 50; let
        callbackProcessed;
 ...
 */
  $.fn.formToArray = function (semantic, elements) {
    ...
    const form = this[0];
    const els = semantic ? form.getElementsByTagName('*') : form.elements;
    if (!els) {
      return a;
    }
    ...
    for (i = 0, max = els.length; i < max; i++) {
      ...
      if (v && v.constructor == Array) {
        ...
        if (elements) elements.push(el);
        a.push({
          name: n, value: v, type: el.type, required: el.required,
        });
      }
    }
 ...
 */
  $.fieldValue = function (el, successful) {
    const n = el.name; const t = el.type; const
    ...
    return this.each(function () {
      ...
      if (re.test(t) || tag == 'textarea') {
        ...
        if ((includeHidden === true && /hidden/.test(t))
                 || (typeof includeHidden === 'string' && $(this).is(includeHidden))) this.value = '';
      }
    });
  };
